...
1-3. The first post introduces a single-agent random walk in Mesa as a baseline model of stochastic movement, showing how a few simple rules can generate complex trajectories and why that matters as a null model for later comparisons. The second expands the setup to multiple agents and emphasizes how individual randomness can produce collective patterns, while also improving the code structure with Mesa’s `DataCollector` and cleaner model architecture. The third shifts from pure randomness to adaptive behavior by adding reinforcement learning, so agents gradually learn to prefer resource cells, avoid toxic ones, and balance exploration with exploitation as their experience accumulates. Taken together, the series moves from randomness, to scale, to learning. That progression gives the blog a clear conceptual arc: random walks as a starting point, multi-agent systems as a way to study collective behavior, and learning agents as a bridge to more realistic adaptive models.

4-5. The next two posts explain how the Schelling segregation model uses simple local decision rules to show that substantial residential clustering can emerge even when agents have only moderate preferences for living near similar neighbours. The first post introduces the classical model: agents occupy a grid, assess the proportion of same-group neighbours, and relocate when that proportion falls below a threshold; through repeated moves and feedback effects, initially mixed populations may develop segregated clusters without any agent explicitly seeking complete separation. It also emphasizes that this is a deliberately simplified model, useful for illustrating emergence but unable to capture institutional forces such as zoning, lending discrimination, employment access, or housing costs. The second post extends this baseline by adding unequal incomes, accumulated wealth, moving costs, and spatially varying rents. In that version, dissatisfaction alone no longer guarantees mobility: an agent must be able to pay both the relocation cost and the rent of an available home. The extension suggests that economic inequality can constrain lower-income households’ choices, create persistent “frustrated” states in which agents remain unhappy but immobile, and reinforce both social and economic segregation over time.

6-7. Together, the two posts use cellular automata to show how spatially local interactions can generate system-level patterns. The first introduces Conway’s Game of Life as a binary grid model in which cells update synchronously according to their Moore-neighborhood counts; it emphasizes how grid structure, boundary conditions, neighborhood definitions, and simple birth–survival rules can produce still lifes, oscillators, gliders, and more irregular dynamics. The second extends that framework from two states to a stochastic three-state ecosystem—empty cells, prey, and predators—where local reproduction, mortality, and predation rules produce population cycles, traveling waves, and sometimes spiral-like spatial patterns. Together, these posts show that spatial topology and update rules shape whether a model stabilizes, oscillates, collapses, or supports coexistence, while also illustrating a sensible modelling progression from simple, interpretable rules toward richer ecological or social applications.


8-9. The two posts develop a progressively richer computational framework for studying forest-fire dynamics and management. The first introduces a cellular-automaton model in which probabilistic tree growth, lightning ignition, and local fire spread generate emergent spatial mosaics, quasi-periodic fire cycles, and threshold-like transitions between localized and landscape-scale burns; it uses these dynamics to examine implications for fuel reduction, firebreak design, climate-driven changes in ignition and spread, and ecological trade-offs. The second extends this ecological model into a spatially explicit multi-agent suppression system by incorporating firefighter agents, water sources, roads, fire intensity, coordinated task assignment, and staged temporal updates. It compares reactive, preventive, and aggressive suppression policies across measures such as burned area, response time, resource utilization, and cost, suggesting that prevention may reduce average fire damage through earlier detection, whereas aggressive response can limit extreme events at greater resource cost. Both posts appropriately position the models as conceptual and comparative tools rather than precise operational forecasts, noting limitations associated with homogeneous landscapes, simplified fire physics, temporal resolution, and the incomplete representation of human decision-making.
